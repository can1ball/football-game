...
bool CImageFile::LoadBitmapFromFile(const char *szFileName, HDC hdc, ULONG width, ULONG height)
{
	BYTE *pData;
	...
	m_hBMP = (HBITMAP)LoadImage(g_hInst, szFileName, IMAGE_BITMAP, width, height, LR_CREATEDIBSECTION | LR_LOADFROMFILE);
	if(!m_hBMP)
		return false;
...
{
	LoadBitmapFromFile(m_szFileName, hdc, width , height);
}
